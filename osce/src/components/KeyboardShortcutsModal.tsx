    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-base font-semibold text-[#1a1a1a] dark:text-zinc-100">Keyboard shortcuts</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
        ...
        <dl className="space-y-2.5">
          {SHORTCUTS.map(({ key, description }) => (
            <div key={key} className="flex items-center justify-between gap-4">
              <dd className="text-sm text-[#4a4a4a] dark:text-zinc-300">{description}</dd>
              <dt>
                <kbd className="rounded border border-[#d0d0d0] bg-[#f5f5f5] px-2 py-0.5 font-mono text-xs text-[#1a1a1a] shadow-[0_1px_0_#c0c0c0] dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 dark:shadow-[0_1px_0_#404040]">
                  {key}
                </kbd>
              </dt>
            </div>
